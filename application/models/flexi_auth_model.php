...
class Flexi_auth_model extends Flexi_auth_lite_model {
	...
	public function verify_password($identity, $verify_password){
		...
		$ret		 = $hash_token->CheckPassword($database_salt.$verify_password.$static_salt, $database_password);
		return $ret;
	}
	###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###
	/**
	...
	public function forgotten_password($identity){
		...
		$this->data['message']	 = "We're sorry, something went wrong.";
		if($ret){
			$this->data['message'] = "An email has been sent to $identity, please follow the directions there.";
		}
		return $ret;
	}
	###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###
...
//		die('umhere');
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE){
		...
		$sql_select = array(
			...
			$this->auth->tbl_col_user_account['upro_first_name'],
			$this->auth->tbl_col_user_account['upro_last_name']
		);
	...
	private function set_login_sessions($user, $logged_in_via_password = FALSE){
		...
		//Set User First Name and Last Name to session or identifier if none set


		$name = $user->upro_first_name.' '.$user->upro_last_name;
		if(trim($name) == ""){
			$name = $user->uacc_username;
		}

		$this->auth->session_data['user_info'] = $name."($user->uacc_id)";
		###+++++++++++++++++++++++++++++++++###
